...
- Library styles in `projects/hviktor/src/styles.css` import `@digdir/designsystemet-css` followed by the generated theme `./design-tokens-build/hvikt.css`. Custom overrides go in scoped `@layer` blocks.
- The theme is built from `designsystemet.config.json` (repo root) via `npm run tokens`. `design-tokens/` is the committed source Tokens Studio syncs against; `projects/hviktor/src/design-tokens-build/` is a gitignored build artifact regenerated by `prebuild:lib`. Never edit either directly — change the config file.
